...
const NewSitePopup = forwardRef<HTMLDialogElement, MissionPopupProps>(
  (props, ref) => {
    ...
    const [currentPriorityIndex, setCurrentPriorityIndex] = useState<
      0 | 1 | 2 | 3
    >(1);
    const [formValues, setformValues] = useState<ReqNewSite>({
      title: "",
      priority: 0,
      description: "",
      emails: [],
    ...
    const handleCreateNewSite = async (e: FormEvent) => {
      e.preventDefault();
      ...
      try {
        const response = await fetch(
          `${baseUrl}/new-site/create-new-site`,
          {
            method: "POST",
            headers: {
