...
const NewSites = () => {
  ...
  const newSiteDialogRef = useRef<HTMLDialogElement>(null);
  ...
  const handladdMissionButtonClick = () => {
    const dialog = newSiteDialogRef.current;
    if (dialog) {
      dialog.style.display = "flex";
      newSiteDialogRef.current?.showModal();
    }
  };
  ...
  const fetchNewSites = async (
    ...
    const url = status
      ...
      : month
      ? `${baseUrl}/new-site/get-new-sites-by-date/${month}/${year}?offset=${offset}&limit=${limit}`
  ...
  return (
    <div className="flex w-full md:h-[100vh]">
      ...
      <NewSitePopup ref={newSiteDialogRef} fetchNewSites={fetchNewSites} />
      <DeletePopup
        page="new site"
        ref={deleteDialogRef}
